...
export default class AddMctInfo extends EventComponent {
    ...
    componentWillMount() {
        // Ajax("mctManageController/getMctShopInfo.do",
        //     {}).then(v => {
        //     if (true === v.success) {
        //         this.setState({
        //             data:v.result,
        //             originPath:v.result["mctLogoUrl"],
        //         })
        //     } else {
    ...
    updatePic(image) {
        ...
        Ajax("MctTradeRemarkController/updMctLogo.do",
            {}, [{name: "file", path: image.path, mime: image.mime}]).then(v => {
            if (true === v.success) {
    ...
    _isTrue() {
        if (this.state.names != null
            && this.state.phone != null&& this.state.names != '' && this.state.phone != ''
        ) {
            return false;
        } else {
            return true;
    ...
    handerRegisetClick() {
        Ajax("ConsumerController/consRegister.do", {
        ...
        return <View style={{height: Theme.size.height, backgroundColor: Theme.color.background}}>
            ...
            >
                {
                    titleList.map((l, i) => {
                        ...
                        return <Component
                            ...
                            imgStr={this.state.select ? this.state.imgPath : ""}
            ...
            <View style={styles.center}>
                <View style={styles.itemInput}>
                    ...
                    <TextInput style={styles.textInput} placeholder="请输入商户名"
                               value={this.state.names}
                               placeholderTextColor={"#dfdfdf"}
                               onChangeText={(namesValue) => {
                                   this.setState({names: namesValue});
                               }}/>
                </View>
                <View style={styles.itemInput}>
                    <Text style={styles.itemTitles}>
