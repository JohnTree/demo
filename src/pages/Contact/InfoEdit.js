...
export default class InfoEdit extends EventComponent {
    ...
    constructor(props) {
        super(props);
        var name=DataBus.getAndClear("conName");
        var phone=DataBus.getAndClear("conPhone");
        var imgPath=DataBus.getAndClear("headUrl");
        this.forceUpdate();
        this.state = {
            conName: name,
            conPhone:phone ,
    ...
    handerRegisetClick() {
        Ajax("CmgInfoRemakeController/updConcatInfo.do", {
            conName: this.state.conName,
            conPhone: this.state.newPhone,
            oldPhone:this.state.conPhone,
            headUrl:this.state.imgPath==""?this.state.originPath:this.state.imgPath
        }).then(v => {
            if (true === v.success) {
    ...
    render() {
        ...
        return <View style={{height: Theme.size.height, backgroundColor: Theme.color.background}}>
            <List containerStyle={styles.InfoEdit} scrollEnabled={true}
            >
                {
                    titleList.map((l, i) => {
                        ...
                        if (rightTitle && l.key != "headUrl" && rightTitle.length > 40) {
                            rightTitle = rightTitle.slice(0, 40) + "...";
                        }
                        let props={
                            rightTitle:this.state.originPath
                        }
                        ...
                        return <Component
                            ...
                            imgStr={this.state.select ? this.state.imgPath : this.state.originPath }
                ...
                <View style={styles.itemInput}>
                    ...
                    <TextInput style={styles.textInput} placeholder="请输入电话"
                               ...
                               onChangeText={(phoneValue) => {
                                   this.setState({newPhone: phoneValue});
                               }}/>
                </View>
            </View>
            <Button
